...
<script>
    //here we continue taking the user data to fill their profile, we have the user id in the session
    //use it to query to and from the database
    export let data;
</script>
<head>
    <title>
        Onboarding | fundi4All
    </title>
</head>

    <form method="post" action="/auth?/updateUser" class="card shadow flex flex-col gap-2 items-start p-10 lg:border w-full lg:w-2/5">
